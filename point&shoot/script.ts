...
const collsionCanvas = document.getElementById(
  "collisionCanvas"
...
class Raven {
  ...
  public maxFrame: number = 6;
  public timeSinceFlap: number = 0;
  ...
  update(delta: number) {
    ...
    if (this.y <= 0 || this.y + this.destWidth > CANVAS_HEIGHT)
      this.directionY *= -1;
    if (this.x + this.destWidth < 0) this.isAnimationComplete = true;
...
class Explosion {
  ...
  public srcHeight: number;
  public frame: number = 0;
  public maxFrame: number = 5;
  public sound: HTMLAudioElement;
  public timeSinceLastPlay: number = 0;
  ...
  update(delta: number) {
    ...
    if (this.timeSinceLastPlay > this.frameInterval) {
      this.frame++;
      if (this.frame >= this.maxFrame) this.isAnimationComplete = true;
...
class Particle {
  ...
  public maxRadius: number = Math.random() * 20 + 35;
  public isAnimationComplete: boolean = false;
  public speedX: number = Math.random() * 1 + 0.5;
  constructor(
    public x: number,
    public y: number,
    public size: number,
  ...
  update() {
    this.x += this.speedX;
    this.radius += 0.5;
    if (this.radius > this.maxRadius) this.isAnimationComplete = true;
  }
